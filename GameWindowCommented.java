...
public class GameWindowCommented implements MouseListener
{	
	//declaring variables
	...
	public GameWindowCommented()
	{
		...
		//setting initial game status variables
		gameStarted = false;
		gameOver = false;
		singlePlayer = false;
		...
		//Arranging the colored squares on the screen
		for (int x=0; x < 8; x++)
		{	 
	...
	//Method called by the main menu that activates colorblind mode
	...
	@Override
	//mouseReleased method which advances the game
	public void mouseReleased(MouseEvent e) {
		int x,y;
		PanelSquare clicked = (PanelSquare)e.getSource(); //saving the clicked square
		...
		//Activating the starting piece. This only runs once per game.
		if (!gameStarted){
			if (x == 0 || x == 7){
		...
		//blockedByComputer can only be true in single-player if the human player is blocked
		if (!blockedByComputer){
			...
			//moving the piece in the model
			from = model.getSquarePieceIsOn(p);
			gameOver = model.move(p, s);

			//moving the piece in the gui
			fromPS = board.panelSquares[from.getY()][from.getX()];

			movedPiece = fromPS.getLabel();

			//clearing the piece's initial square
			fromPS.remove(0);
			fromPS.repaint();

			//replacing the symbol on the square if colorblind mode is active
			if(colorBlind)
			{
		...
		//This else block only activates in single player mode. This is where the computer player acts.
		else{
			allowable = model.getAllowableMoves(p);
			//in case of blocked piece, switch p again to the other player
			if (allowable.size() == 0){
			...
			if (!gameOver){
				...
				//getting the allowable moves for the human's piece
				nextPlayer = p.getPlayer() == Player.WHITE?Player.BLACK:Player.WHITE;
				c = model.getSquareColor(next);
				p = new Piece(nextPlayer,c);
				allowable = model.getAllowableMoves(p);

				//checking that the human is not blocked, and drawing the available moves if not blocked
				if (allowable.size() != 0){
					blockedByComputer = false;
					for ( Square as: allowable){
						allowablePanel = board.panelSquares[as.getY()][as.getX()];
				...
				//otherwise, the computer player moves again.
				else {
	...
	//the single player button, used for debugging. Inactive in final code.
